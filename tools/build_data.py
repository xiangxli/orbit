...
# Milky Way background: NASA SVS 4851 "Deep Star Maps 2020", milkyway_2020_4k.exr
# (public domain, celestial J2000 coordinates, plate carrée, centred on 0h RA,
# RA increasing to the left). Convert to sRGB JPEG first:
#   ffmpeg -apply_trc iec61966_2_1 -i milkyway_2020_4k.exr -q:v 5 milkyway.jpg
# Embedded as a data URL so WebGL can use it from file:// (a plain <img> from
# file:// is treated as cross-origin and would taint the texture).
mw = os.path.join(DATA_DIR, 'milkyway.jpg')
if os.path.exists(mw):
    import base64
    b64 = base64.b64encode(open(mw, 'rb').read()).decode('ascii')
    with open(os.path.join(ROOT, 'milkyway.js'), 'w', encoding='utf-8') as f:
        f.write('// Generated by tools/build_data.py -- do not edit by hand.\n'
                '// Milky Way background: NASA/Goddard Space Flight Center Scientific Visualization Studio,\n'
                '// "Deep Star Maps 2020" (https://svs.gsfc.nasa.gov/4851), public domain. Celestial J2000\n'
                '// coordinates, plate carree centred on RA 0h with RA increasing to the left.\n'
                'var MILKYWAY_DATA_URL = "data:image/jpeg;base64,' + b64 + '";\n')
    print(f'milkyway.js: {os.path.getsize(os.path.join(ROOT, "milkyway.js")) / 1024:.0f} KB')
else:
    print('milkyway.jpg not found in data dir; milkyway.js not regenerated')
